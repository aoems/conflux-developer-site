...
* 运行 conflux
打开开始菜单, 搜索 cmd, 双击打开:
输入命令行：`cd conflux\run`
复制代码conflux --config default.toml 2>stderr.txt，回车，挖矿开始。

* 停止Conflux，即stop mining
- 在运行 conflux 的 cmd 窗口按： ctrl +c , 或者- 直接关闭cmd窗口，或者- 打开任务管理器：详细信息--> 查找 conflux， 选择之后结束进程
